...
class UserService:
    ...
    def create_user(self, user: UserRegistrationSchema):
        if (not self.repo.get_by_username(user.username) and
            not self.repo.get_by_email(user.email)):
    ...
    def get_user_by_username(self, username: str):
        db_user = self.repo.get_by_username(username)
        if not db_user:
            raise HTTPException(status_code=404, detail="Пользователь не существует")

        return db_user

    def authenticate_user(self, credentials: UserLoginSchema):
        db_user = self.get_user_by_username(credentials.username)
